...
**M16-E 已交付**：为卡面补充六旌／谋策印记，在烽垒中展示双方成阵进度与完整阵型摘要，在行动令中显示选牌和合法目标数量，并为移动端增加九垒快速定位。摘要复用公开投影与既有阵型求值器，不新增规则事实来源。

**M16-F 已交付**：将双阵使席位、统一牌背、成阵摘要、烽垒归属、选牌反馈和移动端九垒导航接入在线房间与权威投影棋盘。所有展示只读取既有 `PlayerView` 与 `legalCommands`，未修改 M14/M15 协议和房间服务。

**M16-G 已交付**：将阵使印记、主题背景和九垒归属扩展到私密交接、AI 推演、公开回放与终局复盘；交接和活动回放继续只使用公开状态／观战投影，终局关键烽垒来自既有 `GameSummary`，未改变隐私边界或档案格式。

**M16-H 已交付**：将内容包卡牌描述接入完整／紧凑卡面的可访问名称，并在本地、AI 和在线军令区展示选牌效果、合法目标数量与取消入口。描述不参与判定，目标数量仍来自既有 `legalCommands`。

**M16-I 已交付**：将中英文规则知识接入游戏内双语战场手册，覆盖胜利、回合、五类阵型、夺垒证明、谋策分类与特殊限制。语言切换只影响规则展示，不改变内容包 ID、规则状态或用户存档。

**M16-J 已交付**：为本地、AI 与在线棋盘加入数字键 `1`–`9` 烽垒命令和 `Escape` 取消选牌，操作只消费既有 `legalCommands`，并在输入控件、抽屉、交接、AI 与终局状态下停用。

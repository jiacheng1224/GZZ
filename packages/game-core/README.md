...
M16 起，玩家可见的原创名称、术语与主题统一由 `packages/game-content` 提供。本包中的颜色、战术牌和阵型 ID 继续作为协议与存档兼容所需的稳定内部标识，不应直接显示给玩家。
...
R2 规则闭环、R3 Web 对局、R4 教学回放与 R5 单人体验均已完成。R6 已完成 M14 协议、M15-A 房间状态机、M15-B1 持久化 API 与 M15-B2 在线客户端；当前等待部署联调和弱网出口验证。M16-B 的视觉语汇与认知研究工具已在独立内容包和 UI 中接入，不改变本包的规则语义。
